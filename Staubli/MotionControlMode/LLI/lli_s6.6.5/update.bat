...
REM Management choice lli / VAL3
if "%l_mode%"=="lli" GOTO TARGET
if "%l_mode%"=="val3" GOTO TARGET
GOTO ERROR
...
IF NOT EXIST _VAL3ftp.txt set l_file=_VAL3ftp.txt
IF NOT EXIST _VAL3ftpCS8C.txt set l_file=_VAL3ftpCS8C.txt
...
IF NOT EXIST flash\sys\val3.out set l_file=flash\sys\val3.out
...
IF NOT EXIST flash\usr\configs\iomap.cf set l_file=flash\usr\configs\iomap.cf
...
GOTO CALL_FTP
:VAL3
IF "%l_target%"=="teknor" type _VAL3ftp.txt | _replaceStr #TEMP# %l_temp% | _replaceStr #LOGIN# %l_login% | _replaceStr #PASSWORD# %l_password% | _replaceStr #TARGET# %l_target% > %l_temp%\ftp.txt
IF NOT "%l_target%"=="teknor" type _VAL3ftpCS8C.txt | _replaceStr #TEMP# %l_temp% | _replaceStr #LOGIN# %l_login% | _replaceStr #PASSWORD# %l_password% | _replaceStr #TARGET# %l_target% > %l_temp%\ftp.txt
...
ECHO update mode ip_adress login ftpPassword [target]
ECHO mode = lli / val3
