...
static void _state(LLI_RobotId x_robotId)
{
	...
	for (l_jnt=0; l_jnt<l_jntNb; l_jnt++)
		if((l_isScara)&&(l_jnt==SCARA_LINEAR_JNT))
			printf("  %f", l_jntCmd[l_jnt].m_pos*_M_TO_MM);
		else
			printf("  %f", l_jntCmd[l_jnt].m_pos*_RAD_TO_DEG);
	printf("\nvel cmd : ");
	for (l_jnt=0; l_jnt<l_jntNb; l_jnt++)
		if((l_isScara)&&(l_jnt==SCARA_LINEAR_JNT))
			printf("  %f", l_jntCmd[l_jnt].m_vel*_M_TO_MM);
		else
			printf("  %f", l_jntCmd[l_jnt].m_vel*_RAD_TO_DEG);
	printf("\npos fbk : ");
	for (l_jnt=0; l_jnt<l_jntNb; l_jnt++)
		if((l_isScara)&&(l_jnt==SCARA_LINEAR_JNT))
			printf("  %f", l_jntFbk[l_jnt].m_pos*_M_TO_MM);
		else
			printf("  %f", l_jntFbk[l_jnt].m_pos*_RAD_TO_DEG);
	printf("\nvel fbk : ");
	for (l_jnt=0; l_jnt<l_jntNb; l_jnt++)
		if((l_isScara)&&(l_jnt==SCARA_LINEAR_JNT))
			printf("  %f", l_jntFbk[l_jnt].m_vel*_M_TO_MM);
		else
			printf("  %f", l_jntFbk[l_jnt].m_vel*_RAD_TO_DEG);
	printf("\npos err : ");
	for (l_jnt=0; l_jnt<l_jntNb; l_jnt++)
		if((l_isScara)&&(l_jnt==SCARA_LINEAR_JNT))
			printf("  %f", l_jntFbk[l_jnt].m_posErr*_M_TO_MM);
		else
			printf("  %f", l_jntFbk[l_jnt].m_posErr*_RAD_TO_DEG);
	printf("\ntorque  : ");
	for (l_jnt=0; l_jnt<l_jntNb; l_jnt++)
		printf("  %f", l_jntFbk[l_jnt].m_torque);
	printf("\n\n");
}
...
// _recorder
// ----------------------------------------------------------------
static void _recorder(LLI_RobotId x_robotId)
{

	char l_paramsPath[50];
	char l_recordFile[50];

#ifdef VXWORKS
	strcpy(l_paramsPath,"/usr/recorder/");
	strcpy(l_recordFile,"/usr/recorder/recordSample.dat");
#else if WIN32
	strcpy(l_paramsPath,".\\usr\\recorder\\");
	strcpy(l_recordFile,".\\usr\\recorder\\recordSample.dat");
#endif

	//recorder parameters file must be called records.cfx

	if(LLI_recInit(x_robotId,l_paramsPath) != LLI_OK)
	{
	...
	printf("Press a key to stop record\n");
	getchar();
	...
	if(LLI_recStore(x_robotId,l_recordFile) != LLI_OK)
	{
...
void menu(void)
{
	...
	while((_robotId != NULL)&&(l_choice != 'q'))
	{
		...
		switch (l_choice)
		{
			...
			case 'a':	_recorder(_robotId);			break;
			case 'q': 
#ifndef VXWORKS
				l_choice = 0;
#endif
...
void testControl (const char* x_pathSys,const char* x_pathUsr,const char* x_pathLog)
{
	...
	{
		printf("\t\tNo LLI licence found in /sys/configs/options.cfx !!!   \n");
		_stop(&_robotId);
	}
	else
	{
